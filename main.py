...
def send_msg(my_msg, repeat_number):
    for i in range(int(repeat_number)):
        time_wait = random.uniform(3, 5) # 랜덤값 생성
...
def filter_friend(filter_keyword, init_number): # 사람 아이콘 클릭
    try:
        ...
        print('e ', e)
    try: # X 버튼 존재 시 클릭하여 내용 삭제
        click_img(img_path + 'x.png')
    except:
        pass
    time.sleep(1)
   
    click_img_plus_x(img_path+'search_icon.png') # 돋보기 아이콘 클릭
    if filter_keyword == '':
        pyautogui.keyDown('esc')
    else:
...
def click_img_plus_x(imagePath, pixel=0):
    location = pyautogui.locateCenterOnScreen(imagePath, confidence = conf)
    x, y = location
...
def initialize():
    ...
    filter_keyword = input("필터링할 친구 이름. 없으면 enter.  ex) 학생 직장 이름 : ")
